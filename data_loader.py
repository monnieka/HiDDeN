...
'''
def collate_fn(data):
    """Creates mini-batch tensors from the list of tuples (image, targets).
    ...
    Args:
        data: list of tuple (image, caption). 
            ...
            - targets: torch tensor of shape (?); variable length.
    ...
    labels = torch.zeros(len(targets), max(lengths)).long()
    for i, cap in enumerate(targets):
        end = lengths[i]
        labels[i, :end] = cap[:end]        
